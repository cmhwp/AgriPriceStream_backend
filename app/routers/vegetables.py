...
def read_vegetables_by_price_range(
    ...
    name: Optional[str] = Query(None, description="蔬菜名称"),
    kind: Optional[str] = Query(None, description="蔬菜种类"),
    page: int = Query(1, ge=1, description="页码"),
    page_size: int = Query(10, ge=1, le=100, description="每页记录数"),
    sort_by: Optional[str] = Query("date_desc", description="排序方式"),
    current_user: User = Depends(get_current_user),
    ...
    total = vegetables_crud.get_vegetables_by_price_range_count(
        db, min_price=min_price, max_price=max_price, name_filter=name, kind_filter=kind
    )
